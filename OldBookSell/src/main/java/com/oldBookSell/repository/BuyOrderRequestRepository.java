...
public interface BuyOrderRequestRepository extends CrudRepository<BuyOrderRequest, Integer>{
	...
	@Query(value="select buy_order_request_id,book_name,authors,amount,check_status,small_thumbnail,quantity,address, address2,district, postal_code,state,first_name,last_name,mobile_number from buy_order_request b, address a,user_details u where b.address_id = a.id and a.user_id = u.user_id and b.dilevery_person_id=?1 ORDER BY check_status ASC",nativeQuery = true)
